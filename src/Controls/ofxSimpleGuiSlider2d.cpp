...
void ofxSimpleGuiSlider2d::draw(float x, float y) {
    ...
    if(config->rounded)
	{
        roundedRect(0, 0, width, height - config->slider2DTextHeight, config->rectRadius);
	}
	else
	{
	...
	if(config->rounded)
	{
	    roundedRect(0, height-config->slider2DTextHeight, width, config->slider2DTextHeight, config->rectRadius);
	}
	else
	{
